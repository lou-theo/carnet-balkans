...
- Le **dossier d'organisation complet** (références de réservation, codes, montants, soldes à régler sur place, numéros à appeler, options de repli) est la **source de vérité**. Il est tenu à part, hors du dépôt, et n'est pas publié : c'est l'artefact privé **« Dossier Balkans · carnet complet et privé »**. **À charger intégralement avant toute modification de ce dépôt**, sans exception, même pour une retouche qui paraît isolée : le retrouver par son titre (`Artifact` en `action: "list"`), puis le lire par `WebFetch` sur son URL. Il porte le déroulé heure par heure, les arbitrages déjà verrouillés et les faits redressés ; corriger ici sans l'avoir lu, c'est réintroduire une erreur déjà tranchée ou publier un fait que l'amont a démenti.
